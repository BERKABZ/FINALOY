...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    $sql = "SELECT price, quantity FROM `roll` WHERE id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("UPDATE `roll` SET price = ?, quantity = ? WHERE id = ?");
...
<?php if ($product_updated):?>
    <script>
        window.location.href = "ROLL CAKE.php";
    </script>
<?php endif;?>
...
<body>
    <div class="bck">
        <a href="ROLL CAKE.php"> <img src="update.png"></a>
    </div>

    <h2>Update Product</h2>

    <form method="POST" action="rollupdate.php">
