...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['addproduct'])) {
        ...
        if (!empty($productname) && $price !== '' && $quantity !== '') {
            ...
            $sql = "INSERT INTO `roll` (`id`, `productt`, `price`, `quantity`) VALUES ('$id', '$productname', '$price', '$quantity')";
    ...
    $sql = "SELECT id, productt, price, quantity FROM `roll`";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>" . $row["id"] . "</td>";
            echo "<td>" . $row["productt"] . "</td>";
            echo "<td>" . $row["price"] . "</td>";
            echo "<td>" . $row["quantity"] . "</td>";
            // Update button
            echo "<td><a href='rollupdate.php?id=" . $row["id"] . "' class='btn btn-info'>Update</a></td>";
            echo "<td><a href='rolldelete.php?id=" . $row["id"] . "' class='btn btn-danger'>Delete</a></td>";
            echo "</tr>";
        }
    }
