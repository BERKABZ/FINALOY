...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    $sql = "SELECT price, quantity FROM `dedication cake` WHERE id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("UPDATE `dedication cake` SET price = ?, quantity = ? WHERE id = ?");
...
<?php if ($product_updated):?>
    <script>
        window.location.href = "DEDICATION CAKE.php";
    </script>
<?php endif;?>
...
<body>
    <div class="bck">
        <a href="DEDICATION CAKE.php"> <img src="update.png"></a>
    </div>

    <h2>Update Product</h2>

    <form method="POST" action="CATIONupdate.php">
