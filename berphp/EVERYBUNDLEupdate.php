...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    $sql = "SELECT price, quantity FROM `everyday bundle` WHERE id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("UPDATE `everyday bundle` SET price = ?, quantity = ? WHERE id = ?");
...
<?php if ($product_updated):?>
    <script>
        window.location.href = "EVERYBUNDLE.php";
    </script>
<?php endif;?>
...
<body>
    <div class="bck">
        <a href="EVERYBUNDLE.php"> <img src="update.png"></a>
    </div>

    <h2>Update Product</h2>

    <form method="POST" action="EVERYBUNDLEupdate.php">
