...
<body>
    ...
    <h1>PASTRIES ,BREAD & DELICACIES</h1>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['addproduct'])) {
    ...
    if (!empty($productname) && $price !== '' && $quantity !== '') {
        ...
        $sql = "INSERT INTO `pastries` (`id`, `producttt`, `price`, `quantity`) VALUES ('$id', '$productname', '$price', '$quantity')";
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            if ($conn->errno == 1062) {
                echo "Error: Product with this ID already exists. Please choose a different ID.";
            } else {
...
$sql = "SELECT id, producttt, price, quantity FROM `pastries`";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>" . $row["id"] . "</td>";
        echo "<td>" . $row["producttt"] . "</td>";
        echo "<td>" . $row["price"] . "</td>";
        echo "<td>" . $row["quantity"] . "</td>";
        // Update button
        echo "<td><a href='PASTRIESupdate.php?id=" . $row["id"] . "' class='btn btn-info'>Update</a></td>";
        echo "<td><a href='PASTRIESdelete.php?id=" . $row["id"] . "' class='btn btn-danger'>Delete</a></td>";
        echo "</tr>";
    }
}
